...
import math
import pickle
from autonomous_driving_predictor.core_algorithms.geometry import wrap_angle
import os
...
class TokenProcessor:
    ...
    def __init__(self, token_size):
        ...
        self.agent_token_path = os.path.join(module_dir, 'tokens/cluster_token.pkl')
        self.map_token_traj_path = os.path.join(module_dir, 'tokens/map_token.pkl')
        self.noise = False
        self.disturb = False
        self.shift = 5
        ...
        # Advanced Features: Lane Token Configuration
        self.lane_token_count = 100  # LANE_0 to LANE_99
        self.lane_token_bucket_size = 100
        self.init_lane_tokens()
    ...
    def init_lane_tokens(self):
        """Initialize lane token vocabulary"""
        self.lane_token_vocab = {}
        for i in range(self.lane_token_count):
            self.lane_token_vocab[f"LANE_{i}"] = i

    def encode_lane_tokens(self, agent_state, map_data):
        """
        Encode lane position for an agent
        
        Args:
            agent_state: Dictionary containing agent position and other state info
            map_data: Map data containing lane information
            
        Returns:
            lane_token_id: Integer ID of the lane token
        """
        # Get agent position
        position = agent_state["position"]
        
        # Simple lane ID assignment based on position
        # In a real implementation, this would use proper map matching
        # For now, we'll use a simple spatial hashing approach
        x, y = position[0], position[1]
        
        # Create a simple lane ID based on spatial coordinates
        # This is a simplified approach - in practice you'd use proper map matching
        lane_id = int((x + y) * 10) % self.lane_token_count
        
        return self.lane_token_vocab[f"LANE_{lane_id}"]

    def get_closest_lane(self, position, map_data=None):
        """
        Get the closest lane ID for a given position
        
        Args:
            position: [x, y] position
            map_data: Map data (optional, for future proper implementation)
    ...
    def encode_agent_lane_tokens(self, agent_data, scene_data):
        """
        Encode lane tokens for all agents in the scene
        ...
        Returns:
            lane_tokens: List of lane token IDs for each agent
        """
        lane_tokens = []
        
        # Iterate through all agents in the scene
        for i in range(agent_data['num_nodes']):
            # Get agent position (assuming it's stored in position tensor)
            if 'position' in agent_data:
                position = agent_data['position'][i, self.current_step, :2].cpu().numpy()
            else:
                # Fallback if position not available
                position = [0.0, 0.0]
            
            # Get lane token for this agent
            lane_token_id = self.get_closest_lane(position)
            lane_tokens.append(lane_token_id)
        
        return torch.tensor(lane_tokens, dtype=torch.long)
    ...
    def encode_agent_lane_tokens(self, agent_data, scene_data):
        ...
        Args:
            agent_data: Dictionary containing agent information
            scene_data: Complete scene data including map information
            
        Returns:
            lane_tokens: Tensor of lane token IDs for each agent
        """
        num_agents = agent_data['num_nodes']
        lane_tokens = torch.zeros(num_agents, dtype=torch.long)
        
        # Get agent positions at current timestep
        positions = agent_data['position'][:, self.current_step, :2]  # [num_agents, 2]
        
        for i in range(num_agents):
            if agent_data['valid_mask'][i, self.current_step]:
                position = positions[i]
                lane_id = self.get_closest_lane(position, scene_data)
                lane_tokens[i] = self.lane_token_vocab[f"LANE_{lane_id}"]
    ...
    def tokenize_agent(self, data):
        ...
        # Advanced Features: Add lane tokens
        lane_tokens = self.encode_agent_lane_tokens(data['agent'], data)
        data['agent']['lane_tokens'] = lane_tokens
