...
from autonomous_driving_predictor.evaluation_metrics import average_displacement_error
from autonomous_driving_predictor.evaluation_metrics import final_displacement_error
from autonomous_driving_predictor.evaluation_metrics import token_classification_metric
from autonomous_driving_predictor.model_components import motion_decoder
...
class AutonomousMotionPredictor(pl.LightningModule):

    def __init__(self, model_config) -> None:
        super(AutonomousMotionPredictor, self).__init__()
        ...
        self.init_map_token()
        self.token_path = os.path.join(module_dir, 'tokens/cluster_token.pkl')
        token_data = self.get_trajectory_token()
        self.encoder = motion_decoder.MotionDecoder(
            ...
            use_lane_tokens=getattr(model_config.decoder, 'use_lane_tokens', False),
            use_relational_attention=getattr(model_config.decoder, 'use_relational_attention', False),
            lane_token_embedding_dim=getattr(model_config.decoder, 'lane_token_embedding_dim', 64)
        )
        self.minADE = average_displacement_error.minMultiADE(max_guesses=1)
        self.minFDE = final_displacement_error.minMultiFDE(max_guesses=1)
        self.TokenCls = token_classification_metric.TokenCls(max_guesses=1)
