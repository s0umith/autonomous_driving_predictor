...
class AttentionLayer(MessagePassing):
    ...
    def __init__(self,
        ...
        self.rel_proj = nn.Linear(5, self.num_heads)  
    ...
    def compute_relational_bias(self, positions, velocities):

        rel_pos = positions.unsqueeze(1) - positions.unsqueeze(0) 
        rel_vel = velocities.unsqueeze(1) - velocities.unsqueeze(0) 
        dist = torch.norm(rel_pos, dim=-1, keepdim=True) 
        bias = torch.cat([rel_pos, rel_vel, dist], dim=-1) 
        return self.rel_proj(bias) 
